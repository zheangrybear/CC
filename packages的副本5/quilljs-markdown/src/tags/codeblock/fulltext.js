    ...
    return {
      ...
      action: (text, selection, pattern) => new Promise((resolve) => {
        ...
        setTimeout(() => {
          ...
          this.quillJS.deleteText(startIndex, originalText.length + 1, Quill.sources.USER)
          setTimeout(() => {
            ...
            while (line) {
              ...
              if (isBreak) {
                this.quillJS.deleteText(lineOffset, _text.length, Quill.sources.USER)
                resolve(true)
                return
              }
              this.quillJS.formatLine(lineOffset, 0, 'code-block', true, Quill.sources.USER)
