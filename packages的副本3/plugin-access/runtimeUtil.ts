...
export function traverseModifyRoutes(aRoutes: Routes, access: any): Routes {
  const resultRoutes: Routes = []
    .concat(aRoutes as any)
  ...
  return resultRoutes.map((currentRoute) => {
    ...
    if (currentRoute && currentRoute.access) {
      if (typeof currentRoute.access !== 'string') {
        throw new Error(
          `[plugin-access]: "access" field set in "${currentRoute.path}" route should be a string.`
        );
      }
