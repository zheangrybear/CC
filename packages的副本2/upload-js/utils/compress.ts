import { UploadError, UploadErrorName } from '../errors';
...
class Compress {
  ...
  constructor(file: File, config: CompressOptions) {
    this.file = file;
    this.config = config;
    this.config = {
      quality: 0.92,
  ...
  async process(): Promise<CompressResult> {
    ...
    if (!isSupportedType(this.file.type)) {
      throw new UploadError(UploadErrorName.UnsupportedFileType, `unsupported file type: ${this.file.type}`) as Error;
    }
  ...
  getCanvas(img: HTMLImageElement): Promise<HTMLCanvasElement> {
    return new Promise((resolve, reject) => {
      ...
      if (!context) {
        reject(new UploadError(UploadErrorName.GetCanvasContextFailed, 'context is null'));
        return;
      }
  ...
  async doScale(source: HTMLCanvasElement, scale: number) {
    ...
    if (!mctx || !sctx) {
      throw new UploadError(UploadErrorName.GetCanvasContextFailed, "mctx or sctx can't be null") as Error;
    }
