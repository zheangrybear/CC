...
class MarkdownActivity {
    ...
    if (inputText.length > 1) {
      setTimeout(async () => {
        ...
        for (let v of tokens) {
          ...
          _offset = firstIndex + _targetText.length + 1 + _.filter(line.domNode.childNodes, v => v.localName === 'img').length
        }
      }, 0)
      return
    }
  ...
  async onFullTextExecute (virtualSelection) {
    ...
    const format = this.quillJS.getFormat(formatLineStart, 1)
    if (format['code-block'] || format['code']) {
      // if exists text in code-block, to skip.

      // Following code seems to delete \n in inline code, it seems no need to delete it.
      // if (format['code'] && formatLineStart > 0) {
      //   // ignore all styles when copied text in code block.
      //   const copiedTexts = delta.ops.filter(d => d.insert).map(d => d.insert).join('')
      //   console.info("CODE", copiedTexts)
