...
export default class TableSelection {
  ...
  helpLinesInitial() {
    ...
    LINE_POSITIONS.forEach((direction) => {
      ...
      this[direction].classList.add('xml-ql-selection-line');
      this[direction].classList.add(`xml-ql-selection-line-${direction}`);
      css(this[direction], {
        position: 'absolute',
        display: 'none',
    ...
    const startTd = this.options.evt.target.closest('td[data-row]');
    if (startTd) {
  ...
  mouseDownHandler(e) {
    if (e.button !== 0 || !e.target.closest('.xml-quill-table')) return;
  ...
  repositionHelpLines() {
    ...
    css(this.top, {
      ...
      width: `${this.boundary.width + 3}px`,
      height: '2px',
    });
