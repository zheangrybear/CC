...
const testToken =
  'lVgtk5xr03Oz_uvkzDtQ8LtpiEUWx5tGEDUZVg1y:rAwZ6rnPQbjyG6Pzkx4PORzn6C8=:eyJyZXR1cm5Cb2R5Ijoie1wia2V5XCI6ICQoa2V5KX0iLCJzY29wZSI6InFpbml1LWRhcnQtc2RrIiwiZGVhZGxpbmUiOjE2MTkzNjA0Mzh9';
...
describe('test upload', () => {
  ...
  test('Direct: all api error state.', async () => {
    for (const error of Object.values(errorMap)) {
      ...
      await expect(observablePromisify(upload(File3M, null, testToken))).rejects.toStrictEqual(error);
    }
  });
  ...
  test('Resume: all api error state.', async () => {
    const testApiTable: ApiName[] = ['getUpHosts', 'initUploadParts', 'uploadChunk', 'uploadComplete'];
    ...
    for (const apiName of testApiTable) {
      for (const error of Object.values(errorMap)) {
        ...
        // eslint-disable-next-line no-await-in-loop
        await expect(observablePromisify(upload(File5M, null, testToken))).rejects.toStrictEqual(error);
      }
    }
  });
});
