import type { UploadChunkData } from '../api';
import { initUploadParts, uploadChunk, uploadComplete } from '../api';
import { UploadError, UploadErrorName, UploadRequestError } from '../errors';
import * as utils from '../utils';
import type { Extra, Progress, UploadInfo } from './base';
...
export default class Resume extends Base {
   ...
   */
  private loaded: ChunkLoaded = {} as ChunkLoaded;
  ...
  protected async run() {
    ...
    if (!this.config.chunkSize || !isPositiveInteger(this.config.chunkSize)) {
      throw new UploadError(UploadErrorName.InvalidChunkSize, 'chunkSize must be a positive integer') as Error;
    }

    if (this.config.chunkSize > 1024) {
      throw new UploadError(UploadErrorName.InvalidChunkSize, 'chunkSize maximum value is 1024') as Error;
    }
    ...
    let mkFileResponse: null | utils.ResponseSuccess<any> = null;
    ...
    try {
      ...
      if (error instanceof UploadRequestError && (error.code === 612 || error.code === 400)) {
  ...
  private async uploadChunk(chunkInfo: ChunkInfo) {
    ...
    const response = await uploadChunk(this.token, this.key, chunkInfo.index + 1, this.getUploadInfo(), requestOptions);
  ...
  private async initBeforeUploadChunks() {
    ...
    if (!cachedInfo) {
      ...
      const res = await initUploadParts(this.token, this.bucketName, this.key, this.uploadHost!.getUrl());
