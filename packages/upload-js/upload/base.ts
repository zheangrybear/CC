...
import { UploadError, UploadErrorName, UploadRequestError } from '../errors';
import type { LogLevel } from '../logger';
import type Logger from '../logger';
import * as utils from '../utils';
import type { Host, HostPool } from './hosts';
...
export default abstract class Base {
  ...
  protected uploadAt: number = 0;
  protected progress: UploadProgress = {} as UploadProgress;

  protected onData: (data: UploadProgress) => void;
  protected onError: (err: UploadError) => void;
  ...
  constructor(options: UploadOptions, handlers: UploadHandlers, hostPool: HostPool, logger: Logger) {
    ...
    this.logger.info('config inited.', this.config);
    ...
    this.logger.info('putExtra inited.', this.putExtra);
    ...
    // } catch (error: any) {
    //   this.logger.error('get putPolicy from token failed.', error);
    //   this.onError(error);
    // }
  }
  ...
  protected async checkAndUpdateUploadHost() {
    ...
    const newHost = await this.hostPool.getUp(this.assessKey, this.bucketName, this.config.upprotocol);

    if (newHost == null) {
      throw new UploadError(UploadErrorName.NotAvailableUploadHost, 'no available upload host.') as Error;
    }
  ...
  // 检查并更新冻结当前的 host
  private checkAndFreezeHost(error: UploadError) {
    this.logger.info('check freeze host.');
    if (error instanceof UploadRequestError && this.uploadHost != null) {
  ...
  private handleError(error: UploadError) {
  ...
  public async putFile(): Promise<void> {
    ...
    if (this.file.size > 10000 * GB) {
      this.handleError(new UploadError(UploadErrorName.InvalidFile, 'file size exceed maximum value 10000G'));
      return;
    }
    ...
    if (this.putExtra.customVars) {
      if (!utils.isCustomVarsValid(this.putExtra.customVars)) {
        this.handleError(
          new UploadError(
            UploadErrorName.InvalidCustomVars,
            // FIXME: width => with
    ...
    if (this.putExtra.metadata) {
      if (!utils.isMetaDataValid(this.putExtra.metadata)) {
        this.handleError(new UploadError(UploadErrorName.InvalidMetadata, 'metadata key should start with x-qn-meta-'));
        return;
      }
    }
  ...
  private clear() {
    ...
    this.xhrList.forEach((xhr: XMLHttpRequest) => {
      const temxhr = xhr;
      temxhr.onreadystatechange = null;
      temxhr.abort();
    });
