...
function getImageUrl(key: string, domain: string) {
  ...
  let tarDoman = domain;
  if (tarDoman.slice(tarDoman.length - 1) !== '/') {
    tarDoman += '/';
  }
  return tarDoman + encodeKey;
}

export function imageView2(op: ImageViewOptions, key?: string, domain?: string) {
  if (!/^\d$/.test(String(op.mode))) {
    throw new Error('mode should be number in imageView2');
  }

  const { mode, w, h, q, format } = op;

  if (!w && !h) {
    throw new Error('param w and h is empty in imageView2');
  }
...
export function watermark(op: ImageWatermark, key?: string, domain?: string) {
  const { mode } = op;
  if (!mode) {
    throw new Error("mode can't be empty in watermark");
  }
  ...
  if (mode === 1) {
    const { image } = op;
    if (!image) {
      throw new Error("image can't be empty in watermark");
    }
  ...
  if (mode === 2) {
    ...
    if (!text) {
      throw new Error("text can't be empty in watermark");
    }
...
export function pipeline(arr: Pipeline[], key?: string, domain?: string) {
  ...
  if (isArray) {
    for (let i = 0, len = arr.length; i < len; i += 1) {
      option = arr[i];
      if (!option.fop) {
        throw new Error("fop can't be empty in pipeline");
      }
      switch (option.fop) {
        case 'watermark':
      ...
      if (errOp) {
        throw new Error('fop is wrong in pipeline');
      }
    }

    if (key && domain) {
      imageUrl = `${getImageUrl(key, domain)}?${imageUrl}`;
      const { length } = imageUrl;
  ...
  throw new Error("pipeline's first param should be array");
}
