 ...
 *
 * @param {!Object[]} treeData
 * @param {number|string|undefined} parentKey - The key of the to-be parentNode of the node
 * @param {!function} getNodeKey - Function to get the key from the nodeData and tree index
 * @param {boolean=} ignoreCollapsed - Ignore children of nodes without `expanded` set to `true`
 ...
 */
export function sortNodesUnderParent({
  treeData,
  getNodeKey,
  parentKey = undefined,
  ignoreCollapsed = true,
  ...
  if (parentKey === undefined || parentKey === null) {
    return {
      treeData: _.sortBy(treeData, ['position']),
    };
  }
  const changedTreeData = map({
    ...
    callback: ({ node, path }) => {
      ...
      return {
        ...parentNode,
        children: _.sortBy(parentNode.children, ['position']),
      };
    },
  });
...
export function addNodeUnderParent({
  ...
  const changedTreeData = map({
    ...
    callback: ({ node, treeIndex, path }) => {
      ...
      if (hasBeenAdded || !sameKey(key, parentKey)) {
        return node;
      }
      hasBeenAdded = true;
...
export function getTreeFromFlatData({
  ...
  flatData.forEach((child) => {
    ...
    if (parentKey in parentToChildren) {
      parentToChildren[parentKey].push(child);
    } else {
      parentToChildren[parentKey] = [child];
    }
  });

  if (!(rootKey in parentToChildren)) {
    return [];
  }

  const trav = (parent) => {
    const parentKey = getKey(parent);
    // 如果当前parent的key和parent_key是相同的，则跳出并报错
    if (sameKey(parentKey, getParentKey(parent))) {
      console.warn(`Node ${parent.id} circularly refers itself as parent`);
      return parent;
    }
    if (parentKey in parentToChildren) {
      return {
        ...parent,
        children: parentToChildren[parentKey].map((child) => trav(child)),
      };
    }
  ...
  return parentToChildren[rootKey].map((child) => trav(child));
}
...
export function sameKey(keyA, keyB) {
  if (emptyKey(keyA) && emptyKey(keyB)) {
    return true;
  }
  return keyA === keyB;
}
